...
float * fft1d() {
  ...
  // fourier coefficients must be adjusted by number of points
  int n_points = sizeof(temp_data)/sizeof(double);
  for (size_t i = 0; i < NX/2 + 1; i++) {
    if (i != 0) {
      out_temp_data[i].x *= 2;
      out_temp_data[i].y *= 2;
    }
    out_temp_data[i].x /= n_points;
    out_temp_data[i].y /= n_points;
  }
  ...
  // set fourier to amplitude result
  float * fft_magnitude = new float[NX/2+1];
  for (size_t i = 0; i < NX/2 + 1; i++) {
    fft_magnitude[i] = out_temp_data[i].x*out_temp_data[i].x +
                       out_temp_data[i].y*out_temp_data[i].y;
  }
