...
void fft1d() {
  ...
  if (cufftExecR2C(plan, idata, odata) != CUFFT_SUCCESS) {
    printf("Failed to perform forward fft.\n");
  }
  ...
  // wait (synchronise is failing)
  for (volatile long int i = 0; i < 5000000; i++) {}

  // grab data from graphics and print
  cufftComplex *out_temp_data;
  cudaMemcpy(out_temp_data, odata, (NX/2 + 1)*sizeof(cufftComplex), cudaMemcpyDeviceToHost);
